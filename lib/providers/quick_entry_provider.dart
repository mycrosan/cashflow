...
class QuickEntryProvider extends ChangeNotifier {
  ...
  Future<void> _cancelCurrentOperation() async {
    if (_isOperationInProgress && _currentOperation != null) {
      print('=== QUICK ENTRY PROVIDER: Cancelando operação anterior ===');
  ...
  void _startOperation() {
    ...
    print('=== QUICK ENTRY PROVIDER: Iniciando nova operação ===');
  }
  ...
  void _completeOperation() {
    if (_currentOperation != null) {
      ...
      _currentOperation = null;
      _isOperationInProgress = false;
      print('=== QUICK ENTRY PROVIDER: Operação concluída ===');
    }
  }
  ...
  Future<bool> addQuickTransaction({
    ...
    try {
      ...
      print('=== QUICK ENTRY PROVIDER: Adicionando transação rápida ===');
      ...
      print('=== QUICK ENTRY PROVIDER: Erro ao adicionar transação ===');
      print('Erro: $e');
  ...
  Future<void> loadRecentTransactions({int limit = 10}) async {
    try {
      ...
      print('=== QUICK ENTRY PROVIDER: Carregando transações recentes ===');
      ...
      print('=== QUICK ENTRY PROVIDER: Transações recentes carregadas (${_recentTransactions.length}) ===');
    } catch (e) {
      print('=== QUICK ENTRY PROVIDER: Erro ao carregar transações recentes ===');
      print('Erro: $e');
    ...
    try {
      ...
      return transactions.where((transaction) {
        ...
        return transaction.category.toLowerCase().contains(searchText) ||
               ...
               transaction.associatedMember.name.toLowerCase().contains(searchText);
      }).toList();
    } catch (e) {
